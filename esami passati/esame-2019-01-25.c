...
/*funzione che fa calcola l'altezza */
int altezza (nodo_albero* a) {
	if (a==NULL)
		return -1;
...
/*funzione che fa la visita*/
void visita_albero (nodo_albero* a,int* vett,int prof_att) {
	if (a==NULL)
		return;
	vett[prof_att]++;		//aumenta il vettore 
	visita_albero(a->left,vett,prof_att+1);
	visita_albero(a->right,vett,prof_att+1);
	return;
}


/*funzione che va la visita di un grafo*/
...
int verifica (nodo_albero* a,grafo* g) {
	...
	int* vett2=(int*)calloc(h,sizeof(int));		//vettore dove ci sta il numero di nodi in ogni altezza
	visita_albero(a,vett2,0);	//nel vettore inserisco i vari nodi es in vett[0]ci sta 1 vett[1] ci stanno 2 nodi e cosi via
	
	for(int i=1;i<=colore;i++) {
		printf("\nvett[%d]=%d ",i,vett[i]);
		for(int j=0;i<h;j++) {
			printf("\naltezza[%d]=%d",j,vett2[j]);
			if(vett[i]==vett2[j])
				return 1;
		}
	}
