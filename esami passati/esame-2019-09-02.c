...
int soloUnFiflio (nodo_albero* a) {
	int out=0;
	if(a==NULL)
		return out;
	if(((a->left==NULL) && (a->right!=NULL)) || ((a->right==NULL) && (a->left!=NULL)))
		out++;
	return out+soloUnFiflio(a->left)+soloUnFiflio(a->right);
}
...
int verifica (nodo_albero* a,grafo* g) {
	...
	int soloUnFiglio=soloUnFiflio(a);
	if(min==soloUnFiglio)
		return 1;
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	int nodi1=soloUnFiflio(alb);
	printf("Numero di nodi con un solo figlio %d",nodi1);
