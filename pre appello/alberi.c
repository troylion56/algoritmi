...
nodo_albero* costrusci_albero () {
	...
	nodo_albero* h=inserimentofilgioright(e,12,'H');					//figlio right di e (h)
	...
	nodo_albero* o=inserimentofilgioright(i,1,'O');
...
int cerca_nodo(nodo_albero* a, int n, int prof) {
    ...
    if (a->info == n) {
        return prof; // nodo trovato
    }
    int prof_left = cerca_nodo(a->left, n, prof+1);
    if (prof_left != -1) {
        return prof_left;
    }
    int prof_right = cerca_nodo(a->right, n, prof+1);
    if (prof_right != -1) {
        return prof_right;
    }
    return -1; // nodo non trovato nei sottoalberi
}
